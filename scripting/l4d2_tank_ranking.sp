...
public void Event_PlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
	...
	{	
		if (!IsPlayerFallen(client))//判断坦克不是倒地状态.
		{
...
void IsTankDamageSort(int client, char[] g_sName)
{
	...
	if (assister_count > 0)
	{
		...
		PrintToChatAll("\x05对\x04坦克%s\x05的伤害排名\x04:\x03%d\x05HP", g_sName, iTankHP[client]);
		for (int x = 0; x < g_iTankRanking; x++)
		{
			...
			{
				//PrintToChatAll("\x04%d\x05:\x03%d%%\x05,\x04伤害量\x05:\x03%d\x05,\x04名字\x05:\x03%N", x + 1, IsTankDamagePercentage(damage, client), damage, attacker);
				FormatEx(g_sRanking[x], sizeof(g_sRanking[]), "%d", x + 1);
				FormatEx(g_sPercent[x], sizeof(g_sPercent[]), "%d", IsTankDamagePercentage(damage, client));
...
//百分比取整.
int IsTankDamagePercentage(int damage, int client)
{
	return RoundToNearest(float(damage)/float(iTankHP[client]) * 100.0);
}
