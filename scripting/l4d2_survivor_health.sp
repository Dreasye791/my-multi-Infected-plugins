...
int iKillDefault[array] = {2, 2, 3, 2, 3, 5, 15, 25};
//这里设置爆头奖励的血量(根据g_sZombieName数组顺序设置).
int iHeadDefault[array] = {3, 3, 5, 3, 5, 8, 25, 35};
...
int    g_iKill[array], g_iHead[array], g_iOneshotWitch, g_iLimitHealth, g_iReviveSuccess, g_iSurvivorRescued, g_iHealSuccess, g_iDefibrillator, g_iOpenHealthPrint;
ConVar g_hKill[array], g_hHead[array], g_hOneshotWitch, g_hLimitHealth, g_hReviveSuccess, g_hSurvivorRescued, g_hHealSuccess, g_hDefibrillator, g_hOpenHealthPrint;
...
public void OnPluginStart()
{
	...
	g_hOneshotWitch	= CreateConVar("l4d2_health_oneshot_witch", "50", "秒杀女巫的幸存者奖励多少血. 0=禁用.", FCVAR_NOTIFY);
	g_hLimitHealth	= CreateConVar("l4d2_survivor_health_Limit", "100", "设置幸存者获得血量奖励的最高上限.", FCVAR_NOTIFY);

	g_hReviveSuccess	= CreateConVar("l4d2_health_reviveSuccess", "5", "救起倒地的幸存者奖励多少血. 0=禁用.", FCVAR_NOTIFY);
	g_hSurvivorRescued	= CreateConVar("l4d2_health_survivorRescued", "10", "营救队友的幸存者奖励多少血. 0=禁用.", FCVAR_NOTIFY);
	...
	g_hOpenHealthPrint   = CreateConVar("l4d2_health_print_enable", "1", "是否打印回血提示. 0=禁用.", FCVAR_NOTIFY);
...
void GetConVarChange()
{
	...
	g_iOpenHealthPrint = g_hOpenHealthPrint.IntValue;
}
